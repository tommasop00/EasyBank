...
std::vector<std::unique_ptr<Transaction>> LogFileManager::getAllTransiction(const std::string &iban) {
    ...
    for (auto log : allLog) {
        ...
        if (arraySplit[0] == iban || arraySplit[2] == iban) {
            float amount = std::stof(arraySplit[1]);
            if (arraySplit.size() == 3) {
                std::unique_ptr<Transaction> t(new Transaction(arraySplit[0], amount, arraySplit[2]));
                ret.push_back(std::move(t));
            } else {
                std::unique_ptr<Transaction> t(new Transaction(arraySplit[0], amount));
                ret.push_back(std::move(t));
            }
        }
    }
